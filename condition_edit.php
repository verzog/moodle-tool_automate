...
$PAGE->set_url(new moodle_url('/admin/tool/automate/condition_edit.php',
    ['ruleid' => $ruleid, 'id' => $id]));

$existing = null;
if ($id) {
    $existing = $DB->get_record('tool_automate_condition',
...
if ($mform->is_cancelled()) {
    ...
    if ($existing) {
        ...
        $maxsort = (int) $DB->get_field('tool_automate_condition', 'COALESCE(MAX(sortorder), -1)',
            ['ruleid' => $ruleid]);
